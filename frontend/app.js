...
function renderUserProfile(user) {
  ...
  profileRoundsGrid.innerHTML = user.round_scores.map(rs => {
    const isCurrent = rs.round_id === 'r5';
    const scoreDisplay = rs.score !== null ? `${rs.score} pts` : '<span class="text-slate-500">—</span>';
    const statusText = rs.participated ? 'Participated' : 'Skipped';
    const statusClass = rs.participated ? 'text-emerald-400 bg-emerald-500/10' : 'text-slate-500 bg-slate-800/40';

    return `
      <div class="p-3 rounded-xl border ${isCurrent ? 'bg-indigo-950/40 border-indigo-500/40' : 'bg-slate-950/60 border-slate-800'} space-y-1">
        <div class="flex items-center justify-between">
          <span class="text-[10px] sm:text-[11px] font-semibold text-slate-400 truncate">${rs.round_name}</span>
          ${isCurrent ? '<span class="w-1.5 h-1.5 rounded-full bg-indigo-400"></span>' : ''}
        </div>
  ...
  // Render Round 5 predictions
  const preds = user.r5_predictions || [];
  if (preds.length === 0) {
    ...
    profilePredictionsContainer.innerHTML = `
      <div class="p-6 text-center text-xs text-slate-400 space-y-1">
        <p>Participant did not submit predictions for Round 5 (UCL MD1).</p>
      </div>
    `;
  } else {
              ...
              return `
                <tr class="hover:bg-slate-800/40 transition">
                  ...
                  <td class="py-2.5 px-3 sm:px-4 text-white font-medium text-xs">
                    <div>${p.match}</div>
                    <div class="text-[10px] text-slate-400 sm:hidden mt-0.5">Result: ${outcomeShort[p.actual] || p.actual || '—'}</div>
                  </td>
                  ...
                  <td class="py-2.5 px-3 sm:px-4 text-slate-400 text-xs hidden sm:table-cell whitespace-nowrap">
                    ${outcomeShort[p.actual] || p.actual || '—'}
                  </td>
      ...
      <tr class="${rowClass}" onclick="selectUser('${u.username}')">
        ...
        <td class="py-3 px-3 sm:px-4 sticky-col-2">
          <div class="flex items-center space-x-1.5">
            ...
            ${aliasesNotice}
            ${u.has_r5_predictions ? '<span class="w-1.5 h-1.5 rounded-full bg-emerald-400" title="R5 prediction submitted"></span>' : ''}
          </div>
        </td>
...
function renderRoundTable(roundId, filter) {
  const roundMeta = globalData.rounds.find(r => r.id === roundId) || { name: roundId };
  tableHeaders.innerHTML = `
    <tr>
      ...
      <th class="py-3 px-3 sm:px-4 text-right">Round Points</th>
    </tr>
  `;

  const standings = globalData.round_standings[roundId] || [];
  ...
  tableBody.innerHTML = filtered.map(u => {
    ...
    return `
      <tr class="${rowClass}" onclick="selectUser('${u.username}')">
        ...
        <td class="py-3 px-3 sm:px-4 text-right font-black text-indigo-400 font-mono text-xs sm:text-sm">${u.score}</td>
      </tr>
    `;
  }).join('');
}
...
// ==========================================
// Matches Distribution (UCL MD1)
// ==========================================
function renderMatchDistributions() {
  if (!globalData || !globalData.match_distributions) return;

  const matches = globalData.match_distributions;
  matchesGrid.innerHTML = matches.map((m, idx) => {
    let actualBadge = '';
    if (m.actual) {
